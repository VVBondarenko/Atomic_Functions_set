...
double f_d_fup(double x, int n)
{
	int k;
	double y,z=0.;
	for(k=0;k<n+2;k++)
	{
		//z+=(double)factorial(k-1)/(double)factorial(n)/(double)factorial(k-n-1)*
		z+=(double)(binCoeff(k,n+1)-binCoeff(k-1,n+1))*
			f_fup(2.*x+(double)n/2.+1.-(double)k,n);
	}
	return pow(2.,-n+1)*z;
}

double f_dd_fup(double x, int n)
{
	int k;
	double y,z=0.;
	for(k=0;k<n+2;k++)
	{
		//z+=(double)factorial(k-1)/(double)factorial(n)/(double)factorial(k-n-1)*
		z+=(double)(binCoeff(k,n+1)-binCoeff(k-1,n+1))*
			f_d_fup(2.*x+(double)n/2.+1.-(double)k,n);
	}
	return pow(2.,-n+2)*z;
}
...
double ff_up(double x)
{
	int k,m,mmax,kmax,number;
	double y,psin,k2,psin1,t,pi,epspsin,epsintegr;
	if(fabs(x)>1)
		return 0;
	pi=M_PI;

	number=3;          // Number of up_m
	mmax=10;
	kmax=20;
	...
	for(k=1;k<20;k++)
	{
		...
		{
			t=t/2;
			psin=psin*pow(cos(t),m);
		}
		psin=psin*cos((pi*k)*x);
		y=y+psin;
	}
	return y;
}

double ff_d_up(double x)
{
	//y ′ (x) = 2y(2x + 1) − 2y(2x − 1)
	return 2.*(ff_up(2.*x+1.)-ff_up(2.*x-1.));

}

double ff_dd_up(double x)
{
	//y ′ (x) = 2y(2x + 1) − 2y(2x − 1)
	return 4.*(ff_d_up(2.*x+1.)-ff_d_up(2.*x-1.));

}

double f_fup2(double x)
{
	if(x<=2. && x>=-2.)
		return 36./13.*(ff_up(0.25*x-0.5)-2.*ff_up(0.25*x-0.75)+2.*ff_up(0.25*x-1.)-2.*ff_up(0.25*x-1.25));
	else return 0.;
}

double f_d_fup2(double x)
{
	if(x<=2. && x>=-2.)
		return 36./13.*(ff_d_up(0.25*x-0.5)-2.*ff_d_up(0.25*x-0.75)+2.*ff_d_up(0.25*x-1.)-2.*ff_d_up(0.25*x-1.25));
	else return 0.;
}

double f_dd_fup2(double x)
{
	if(x<=2. && x>=-2.)
		return 36./13.*(ff_dd_up(0.25*x-0.5)-2.*ff_dd_up(0.25*x-0.75)+2.*ff_dd_up(0.25*x-1.)-2.*ff_dd_up(0.25*x-1.25));
	else return 0.;
}
